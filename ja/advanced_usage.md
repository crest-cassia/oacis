...
| Command                    | シミュレータの実行コマンド。リモートホスト上でのパスを絶対パスかホームディレクトリからの相対パスで指定する。|
...
{% capture tips %}
"Command"で指定されたコマンドがシェルスクリプトに埋め込まれて実行されます。
様々なホストで同じように実行できるようにホームディレクトリからの相対パスで指定するとよいです。（例. *~/path/to/simulator.out* ）
一つの実行バイナリである必要はなく、一連のバイナリを実行するシェルスクリプトのパスを指定してもよいです。
