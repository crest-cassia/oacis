class ParameterSetsController < ApplicationController
  ...
  def create
    ...
    if @num_runs > 0
      permitted_params = params[:run].present? ? params.require(:run).permit! : {}
    ...
    @num_runs.times do |i|
      created.each do |ps|
        ...
        permitted_params = params[:run].present? ? params.require(:run).permit(:mpi_procs, :omp_threads, :host_parameters, :priority, :submitted_to) : {}
