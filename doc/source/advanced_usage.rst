...
| xsub(https://github.com/crest-cassia/xsub) というスケジューララッパを経由してTorqueなどにジョブ投入する方法について説明する。
...
| シミュレータを実行するホストの環境変数XSUB_TYPEに"torque"を指定する。（ `export XSUB_TYPE="torque"` ）
| Hostの登録を行なうとスケジューラの変数（使用時間、占有するノード数、使用メモリ量など）がxsubからOACISに通知される。
| Runの作成時にスケジューラの変数の値入力が求められる。
| スケジューラの変数の値は、OACISからxsubを実行するときに引数として渡される。
| xsubはこれらの変数を受け取り、ジョブカード（ジョブスクリプトとは異なる）のヘッダに記述してTorqueの投入コマンド（qsub）を実行する。
